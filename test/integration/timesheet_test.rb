...
class TimesheetTest < ActionDispatch::IntegrationTest
  ...
  test 'set hours upserts a single entry per project and day' do
    ...
    assert_difference 'TimeEntry.count', 1 do
      post timesheet_time_entries_path, params: { project_id: @project.id, date: date, hours: 4 }
    end
    assert_redirected_to timesheet_path

    assert_no_difference 'TimeEntry.count' do
      post timesheet_time_entries_path, params: { project_id: @project.id, date: date, hours: 7.5 }
    end
    assert_equal 7.5, @project.time_entries.find_by(date: date).hours

    assert_difference 'TimeEntry.count', -1 do
      post timesheet_time_entries_path, params: { project_id: @project.id, date: date, hours: 0 }
    end
  end
  ...
  test "cannot log hours on another user's project" do
    ...
    post timesheet_time_entries_path, params: { project_id: other_project.id, date: Date.current, hours: 4 }
    assert_response :not_found
  end

  test 'projects crud' do
    assert_difference 'Project.count', 1 do
      post timesheet_projects_path, params: { project: { name: 'seamflow' } }
    end

    project = @user.projects.find_by(name: 'seamflow')
    patch timesheet_project_path(project), params: { project: { name: 'seamflow2', hourly_rate: '90' } }
    project.reload
    assert_equal ['seamflow2', 90.0], [project.name, project.hourly_rate]

    patch timesheet_project_path(project), params: { project: { name: 'seamflow2', hourly_rate: '' } }
    ...
    assert_difference %w[Project.count TimeEntry.count], -1 do
      delete timesheet_project_path(project)
    end
  end
  ...
  test 'day locations upsert and clear' do
    ...
    post timesheet_day_locations_path, params: { date: date, location_id: location.id }
    ...
    assert_no_difference 'DayLocation.count' do
      post timesheet_day_locations_path, params: { date: date, location_id: other.id }
    end
    assert_equal other.id, @user.day_locations.find_by(date: date).location_id

    assert_difference 'DayLocation.count', -1 do
      post timesheet_day_locations_path, params: { date: date, location_id: 0 }
    end
  end
  ...
  test 'deleting a location removes its day assignments' do
    ...
    assert_difference %w[Location.count DayLocation.count], -1 do
      delete timesheet_location_path(location)
    end
  end
end
