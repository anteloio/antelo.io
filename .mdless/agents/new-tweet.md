...
One invocation = one tweet written to `content/tweets/`.

## Step 1 — Read the voice guide and read existing tweets

```bash
cat .mdless/_voice.md
for f in content/tweets/*.md; do echo "=== $f ==="; cat "$f"; done
```

## Step 2 — Find material

```bash
for f in content/blog/*.md; do echo "=== $f ==="; head -30 "$f"; done
```
...
Save to `content/tweets/YYYY-MM-DD.md`, using the next available weekday as the filename.
